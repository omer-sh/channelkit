...
ChannelKit can expose your local instance to the internet using a [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/) via `cloudflared`. This is required for features like inbound webhooks (Resend, Twilio) and voice calls, which need a publicly reachable URL.
