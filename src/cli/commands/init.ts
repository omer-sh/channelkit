...
import { DEFAULT_CONFIG_PATH, DEFAULT_AUTH_DIR } from '../../paths';
...
export async function initCommand() {
  ...
  try {
    ...
    console.log();
    const configPath = await ask(rl, 'Config file path:', DEFAULT_CONFIG_PATH);
    ...
    if (!existsSync(DEFAULT_AUTH_DIR)) {
      mkdirSync(DEFAULT_AUTH_DIR, { recursive: true });
    }
    ...
    if (channels['whatsapp']) {
      ...
      try {
        ...
        const authPath = join(DEFAULT_AUTH_DIR, `whatsapp-whatsapp`);
